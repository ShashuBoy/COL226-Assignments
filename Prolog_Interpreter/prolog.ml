...
let handle_query prog str = 
  ...
  let ans = Structure.solve (Structure.idty_subst) prog goals in match ans with
    Structure.False -> Printf.printf "false\n"
  | Structure.True sub -> SS.iter (fun a -> Printf.printf "%s - " a ; Structure.print_term (sub a) ) vars
;;
    ...
    if (inp.[(len-1)] <> '.') then
      ...
      if (inp = "halt.") then exit 0
        ...
        if ((String.sub inp 0 2) = "[\"") && ((String.sub inp (len - 3) 3) = "\"].") then
          (*let () = Printf.printf "Reached here" in*)
